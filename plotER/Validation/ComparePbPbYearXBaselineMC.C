...
void ComparePbPbYearXBaselineMC(
    ...
    const char* selectionLabel, const char* outputDirectory)
{
    ...
    if (!tree23 || !tree24) throw std::runtime_error("missing X MC tree");
    for (const auto& variable : kVariables) {
        if (!tree23->GetBranch(variable.name) || !tree24->GetBranch(variable.name))
            throw std::runtime_error(std::string("missing X MC branch: ") + variable.name);
    }
    ...
    if (entries23 <= 0 || entries24 <= 0) throw std::runtime_error("empty X MC selection");
    ...
    for (std::size_t index = 0; index < kVariables.size(); ++index) {
        ...
        h23.SetDirectory(nullptr); h24.SetDirectory(nullptr);

        const double sumw23 = h23.Integral(1, h23.GetNbinsX());
        const double sumw24 = h24.Integral(1, h24.GetNbinsX());
        ...
        TH1D ratio(h23); ratio.SetName(Form("ratio_%s", variable.name));
        ratio.SetDirectory(nullptr); ratio.SetStats(false); ratio.Divide(&h24);
        int skippedRatioBins = 0;
        double maximumRatio = 0.0;
        for (int bin = 1; bin <= ratio.GetNbinsX(); ++bin) {
            if (!(h24.GetBinContent(bin) > 0.0) || !std::isfinite(ratio.GetBinContent(bin))) {
                ratio.SetBinContent(bin, -999.0);
                ratio.SetBinError(bin, 0.0);
                ++skippedRatioBins;
            } else {
                maximumRatio = std::max(maximumRatio,
                    ratio.GetBinContent(bin) + ratio.GetBinError(bin));
            }
        }
        ...
        legend.AddEntry(&h23, "PbPb23 X MC", "lep");
        legend.AddEntry(&h24, "PbPb24 X MC", "lep"); legend.Draw();
        ...
        bottom.cd(); ratio.SetTitle(""); ratio.GetYaxis()->SetTitle("23 / 24");
        ratio.GetYaxis()->SetRangeUser(0.0, std::max(2.0, 1.10 * maximumRatio));
        ...
        if (index) json << ",\n";
        json << "    \"" << variable.name
             << "\": {\"bins\": 15, \"minimum\": " << variable.minimum
             << ", \"maximum\": " << variable.maximum
             << ", \"pb23_sumw_before_normalization\": " << sumw23
             << ", \"pb24_sumw_before_normalization\": " << sumw24
             << ", \"pb23_integral\": " << h23.Integral()
             << ", \"pb24_integral\": " << h24.Integral()
             << ", \"ks_distance\": " << ksDistance
             << ", \"chi2\": " << chi2 << ", \"ndf\": " << ndf
             << ", \"chi2_pvalue\": " << chi2P
             << ", \"skipped_ratio_bins\": " << skippedRatioBins << "}";
    }
    json << "\n  }\n}\n";
