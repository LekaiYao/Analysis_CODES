...
root -l -b -q \
  "${script_dir}/macros/ExportSWeightTree.C+(\"${weighted}\",\"${temp_root}\",\"${temp_manifest}\",\"${data}\",\"ntmix\",\"${fit}\",\"${analysis_tree}_sWeight\",\"nsig1__sw\",\"${analysis_tree}\",\"${implicit_flat_selection}\",\"accepted nominal ppRef X R5-aligned feasibility model; shapes fixed and nsig/nbkg floated in the sPlot refit\",\"${root_version}\",\"${git_commit}\",\"${mc}\",\"${analysis_tree}\",\"preliminary_nominal_splot_for_r5_transfer_closure\",\"${baseline_root}\")"
...
"${python_cmd}" "${script_dir}/workflows/validate_sweight_tree.py" \
