...
generation_command='source /cvmfs/sft.cern.ch/lcg/views/LCG_106/x86_64-el9-gcc13-opt/setup.sh && bash plotER/Validation/workflows/run_add_btrk2dr_to_ppref_psi2s_sweight.sh'
...
root -l -b -q \
  "${script_dir}/macros/ExportSWeightTree.C+(\"${source_splot}\",\"${temp_root}\",\"${temp_manifest}\",\"${source_data}\",\"ntmix\",\"${source_fit}\",\"${analysis_tree}_sWeight\",\"nsig1__sw\",\"${analysis_tree}\",\"${implicit_flat_selection}\",\"accepted nominal ppRef psi(2S) model; existing RooDataSet and nsig1__sw exported without refit\",\"${root_version}\",\"${git_commit}\",\"${source_mc}\",\"${analysis_tree}\",\"${physics_status}\",\"${output_root}\")"

python3 "${script_dir}/workflows/finalize_sweight_manifest.py" \
...
root -l -b -q \
    "${script_dir}/macros/ValidatePsi2SSWeightExport.C(\"${local_new_root}\",\"${local_baseline_root}\",\"${source_splot}\",\"${temp_report}\",\"${baseline_root}\")"
