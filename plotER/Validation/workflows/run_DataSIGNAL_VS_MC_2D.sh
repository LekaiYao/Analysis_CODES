...
# Usage:
#   bash workflows/run_DataSIGNAL_VS_MC_2D.sh [tree] [system] [custom_cut] [reweight_variable] [whichWeight]
#
...
#
# Examples:
#   bash workflows/run_DataSIGNAL_VS_MC_2D.sh ntmix_X3872 ppRef
#   bash workflows/run_DataSIGNAL_VS_MC_2D.sh ntmix_PSI2S ppRef

#   bash workflows/run_DataSIGNAL_VS_MC_2D.sh ntmix_X3872 ppRef '' Btktkpt, usePsi2s
#   bash workflows/run_DataSIGNAL_VS_MC_2D.sh ntmix_X3872 ppRef '' Btktkpt,Bchi2Prob usePsi2s
#   bash workflows/run_DataSIGNAL_VS_MC_2D.sh ntmix_X3872 ppRef '' Btktkpt,Bchi2Prob,Bmu1pt usePsi2s
...
cleanup_aclic() {
  rm -f \
    macros/DataSIGNAL_VS_MC_2D_C.so \
    macros/DataSIGNAL_VS_MC_2D_C.d \
    macros/DataSIGNAL_VS_MC_2D_C_ACLiC_dict_rdict.pcm \
    macros/DataSIGNAL_VS_MC_2D_C_ACLiC_dict.cxx \
    macros/DataSIGNAL_VS_MC_2D_C_ACLiC_linkdef.h \
    macros/DataSIGNAL_VS_MC_2D_C_ACLiC_map
}
...
if [[ ! -f "$SPLOT_WEIGHTS" ]]; then
  ...
  echo "        Run the nominal 1-D validation first: bash workflows/run_DataSIGNAL_VS_MC.sh $TREE $SYSTEM"
  exit 1
fi
...
echo "Running macros/DataSIGNAL_VS_MC_2D.C with:"
echo "  TREE          = $TREE"
echo "  SYSTEM        = $SYSTEM"
echo "  CUT           = $CUT"
echo "  DATA          = $DATA"
...
root -l -b -q "macros/DataSIGNAL_VS_MC_2D.C(\"${DATA}\",\"${MC}\",\"${CUT}\",\"${TREE}\",\"${SYSTEM}\",\"${SPLOT_WEIGHTS}\",\"${REWEIGHT_VARIABLE}\",\"${WEIGHT_FILE}\",\"${WHICH_WEIGHT}\")"
