...
McPlotQuality drawMc(const char* path, const char* category, RooDataSet& data,
                     RooAbsPdf& model, RooRealVar& mass, int bins,
                     const RooFitResult& fit, double sigma1, double sigma2,
                     double fraction, double displayMin = 0.0,
                     double displayMax = -1.0)
{
    // Preserve the established 5 MeV value and add the requested 1 MeV
    // diagnostic over the same configured MC fit range.
    const int fineBins = 5 * bins;
    auto calculateChi2 = [&](int numberOfBins, const char* suffix) {
        std::unique_ptr<RooPlot> chi2Frame(mass.frame(Bins(numberOfBins)));
        data.plotOn(chi2Frame.get(), Name(Form("mc_chi2_%s", suffix)),
                    DataError(RooAbsData::SumW2));
        model.plotOn(chi2Frame.get(), Name(Form("model_chi2_%s", suffix)));
        return chi2Frame->chiSquare(Form("model_chi2_%s", suffix),
                                    Form("mc_chi2_%s", suffix),
                                    fit.floatParsFinal().getSize());
    };
    McPlotQuality quality;
    quality.chi2Ndf5MeV = calculateChi2(bins, "5mev");
    quality.chi2Ndf1MeV = calculateChi2(fineBins, "1mev");
    ...
    std::unique_ptr<RooPlot> frame(mass.frame(Bins(fineBins)));
    ...
    frame->Draw();
    TPaveText stats(0.56, 0.62, 0.93, 0.89, "NDC");
    ...
    std::unique_ptr<RooPlot> pullFrame(mass.frame(Bins(fineBins)));
    ...
    if (displayMax > displayMin) {
        pullFrame->GetXaxis()->SetRangeUser(displayMin, displayMax);
    }
    pullFrame->Draw();
                ...
                double scale, double zApprox)
{
    ...
    legend.AddEntry(frame->findObject("signal"), "MC-shape #psi(2S) signal", "l");
    legend.Draw();
...
void RedrawPb23Psi2SMCFitFineBins(const char* workspacePath,
                                  ...
                                  double displayMax = -1.0)
{
    ...
    auto* fit = dynamic_cast<RooFitResult*>(input.Get("fit_result_mc_pb23"));
    if (!workspace || !fit) {
        throw std::runtime_error("missing PbPb23 MC workspace or fit result");
    }
    auto* data = dynamic_cast<RooDataSet*>(workspace->data("mc_pb23"));
    auto* model = workspace->pdf("mc_signal_pb23");
    auto* mass = workspace->var("Bmass");
    auto* sigma1 = workspace->var("mc_sigma1_pb23");
    auto* sigma2 = workspace->var("mc_sigma2_pb23");
    auto* fraction = workspace->var("mc_fraction_pb23");
    if (!data || !model || !mass || !sigma1 || !sigma2 || !fraction) {
        throw std::runtime_error("incomplete PbPb23 MC workspace content");
    }
    gSystem->mkdir(gSystem->DirName(outputPath), true);
    drawMc(outputPath, "PbPb23", *data, *model, *mass, nominalBins, *fit,
           sigma1->getVal(), sigma2->getVal(), fraction->getVal(),
           displayMin, displayMax);
}

void PbPbPsi2SSimultaneousYearFit(
    const char* key,
    ...
    double a1Min, double a1Max, int massBins)
{
    ...
    RooRealVar mcSigma123("mc_sigma1_pb23", "mc sigma1 pb23", 0.008, 0.0005, 0.05);
    RooRealVar mcSigma223("mc_sigma2_pb23", "mc sigma2 pb23", 0.004, 0.0005, 0.05);
    RooRealVar mcFraction23("mc_fraction_pb23", "mc fraction pb23", 0.4, 0.01, 0.99);
    ...
    std::unique_ptr<RooFitResult> mcFit23(mcSignal23.fitTo(
        mc23, Save(), SumW2Error(false), PrintLevel(-1), Warnings(false), Verbose(false),
        Strategy(2), Hesse(true)));

    RooRealVar mcMean24("mc_mean_pb24", "mc mean pb24", meanInitial, meanMin, meanMax);
    RooRealVar mcSigma124("mc_sigma1_pb24", "mc sigma1 pb24", 0.008, 0.0005, 0.05);
    RooRealVar mcSigma224("mc_sigma2_pb24", "mc sigma2 pb24", 0.004, 0.0005, 0.05);
    RooRealVar mcFraction24("mc_fraction_pb24", "mc fraction pb24", 0.4, 0.01, 0.99);
    ...
    std::unique_ptr<RooFitResult> mcFit24(mcSignal24.fitTo(
        mc24, Save(), SumW2Error(false), PrintLevel(-1), Warnings(false), Verbose(false),
        Strategy(2), Hesse(true)));
    ...
    const McPlotQuality mcQuality23 = drawMc(Form("%s/pb23_mc_template_fit.pdf", outputDirectory),
        "PbPb23", mc23, mcSignal23, mass, massBins, *mcFit23,
    ...
    const McPlotQuality mcQuality24 = drawMc(Form("%s/pb24_mc_template_fit.pdf", outputDirectory),
        "PbPb24", mc24, mcSignal24, mass, massBins, *mcFit24,
    ...
    RooWorkspace workspace("ws_psi2s_simultaneous_years", "ws_psi2s_simultaneous_years");
    ...
    json << std::setprecision(17)
         ...
         << "  \"toy_count\": 0,\n"
         << "  \"mc_chi2_policy\": \"5 MeV and 1 MeV values are record-only over the configured MC fit range; chi2_ndf is the backward-compatible 5 MeV alias\",\n"
    ...
    std::cout << "[Psi2S simultaneous] " << key << " DATA23/24="
