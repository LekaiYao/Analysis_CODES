...
INPUT_DIR="${REPO_DIR}/../XGBoost/output/selected/X_pp24_v4_fid3_8v2_rw0_xgb_v1"
DATA="${INPUT_DIR}/DATA_with_score.root"
MC="${INPUT_DIR}/MC_with_score.root"
RESULT_BASE="${SCRIPT_DIR}/results/ppRef/x_rw0_cut_scan/8v2"
ROOT_BASE="${SCRIPT_DIR}/ROOTfiles/ppRef/x_rw0_cut_scan/8v2"
...
SUMMARY="${RESULT_BASE}/rw0_cut_scan_summary.csv"
STABILITY_SUMMARY="${RESULT_BASE}/rw0_cut_scan_stability_summary.csv"
...
run_one() {
    ...
    root -l -b -q \
        "diagnostics/ExportXWorkingPointSummary.C(\"${root_dir}/fit.root\",\"${row}\",\"${label}\",-1,${cut},\"${DATA}\",\"${MC}\",\"${fit_status}\",${root_status})" \
        >>"${log}" 2>&1
    [[ $? -eq 0 && -s "${row}" ]] || return 1
