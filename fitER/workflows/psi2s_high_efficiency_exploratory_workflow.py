...
def modules(repo):
    ...
    from fitER.workflows import psi2s_fit_scan_workflow as base
    from fitER.workflows import psi2s_data_gaussian_workflow as gaussian
...
def prepare(repo, manifest_path, config_path, output_dir):
    ...
    status = base.run_root(
        repo / "fitER/cache/PreparePsi2SFitScanCache.C", expression,
...
def fit_point(repo, manifest_path, config_path, output_dir, key):
    ...
    status = base.run_root(
        repo / "fitER/models/PbPbPsi2SDataGaussianFit.C", expression,
        point_dir / "fit.log",
    )
