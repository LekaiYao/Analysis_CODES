...
validation_macro="${repo_dir}/plotER/Validation/macros/DataSIGNAL_VS_MC.C"
...
for label in S2 S3; do
    ...
    "${root_bin}" -l -b -q \
      "${fit_dir}/diagnostics/ExportPsi2SH004Summary.C(\"${model}\",\"${cuts[${label}]}\",\"${label}\",\"${quality}\",\"${fit_dir}/results/${system}/fit_summary.csv\")"
done
