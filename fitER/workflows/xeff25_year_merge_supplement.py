...
def run(base, output, resume=False):
    ...
    if not (resume and merged_root.is_file() and merged_json.is_file()):
        status = run_root(repo / "fitER/cache/PrepareXMergedFitCache.C", expression, output / "prepare_merged.log")
    ...
    for key, (data_path, tree, selection) in fit_specs.items():
        ...
        status = run_root(repo / "fitER/models/PbPbXDataGaussianFit.C", fit_expression, fit_dir / "fit.log")
