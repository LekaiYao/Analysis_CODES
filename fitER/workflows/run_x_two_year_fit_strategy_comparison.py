...
def single_fit(repo, manifest, cache_root, output_dir, point, year):
    ...
    status = run_root(repo / "fitER/models/PbPbXEfficiencyFit.C", expression,
                      output_dir / "fit.log")
...
def main(argv=None):
    ...
    status = run_root(
        repo / "fitER/cache/PrepareXDataEntryNormalizedMergedCache.C",
    ...
    status = run_root(repo / "fitER/models/PbPbXEfficiencyFit.C", merged_expression,
