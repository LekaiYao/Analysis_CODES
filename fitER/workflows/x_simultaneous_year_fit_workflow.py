...
def prepare(repo, manifest_path, manifest, output_dir):
    ...
    for category in CATEGORIES:
        ...
        status = run_root(repo / "fitER/cache/PrepareXFitScanCache.C", expression, output_dir / f"prepare_{category}.log")
...
def fit_point(repo, manifest_path, manifest, output_dir, key):
    ...
    status = run_root(repo / "fitER/models/PbPbXSimultaneousYearFit.C", expression, point_dir / "fit.log")
...
def main(argv=None):
    ...
    args=parser.parse_args(argv); repo=Path(__file__).resolve().parents[2]; manifest_path=args.manifest.resolve(); output_dir=args.output_dir.resolve(); manifest=load_manifest(manifest_path)
