...
while IFS=$'\t' read -r background_key ml_type target train_tag threshold data_path data_tree selection; do
  ...
  (
    cd "${fit_dir}"
    "${root_bin}" -l -b -q \
      "diagnostics/PbPbH012BackgroundFits.C++(\"${background_key}\",\"${data_path}\",\"${data_tree}\",\"${selection}\",\"${point_dir}\")"
...
python3 "${fit_dir}/workflows/prepare_pbpb24_x_h012.py" aggregate-background \
  --repo "${repo_dir}" --output-dir "${output_dir}"
