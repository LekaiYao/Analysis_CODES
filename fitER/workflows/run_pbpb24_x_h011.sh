...
python3 "${fit_dir}/workflows/prepare_pbpb24_x_h011.py" prepare \
...
run_point() {
  ...
  (
    cd "${fit_dir}"
    "${root_bin}" -l -b -q \
      "diagnostics/PbPbH011InjectionToys.C++(\"${key}\",\"${model_type}\",\"${data_path}\",\"${data_tree}\",\"${reference_path}\",\"${reference_tree}\",\"${selection}\",${yield_minus},${yield_central},${yield_plus},${toys_per_ensemble},${seed_base},\"${point_dir}\",${run_asimov},${run_toys})"
...
python3 "${fit_dir}/workflows/prepare_pbpb24_x_h011.py" check-asimov \
  --repo "${repo_dir}" --output-dir "${output_dir}"
...
python3 "${fit_dir}/workflows/prepare_pbpb24_x_h011.py" aggregate \
  --repo "${repo_dir}" --output-dir "${output_dir}"
