#!/usr/bin/env python3
"""PREPARE/FIT/AGGREGATE nodes for the two-year X MC-shape nominal contract."""
...
def load_manifest(path):
    from scripts.submit_x_mc_shape_simultaneous_year_fit_manifest import load_task
...
def run_root(source_macro, expression, log_path):
    scratch = Path(os.environ.get(
        "_CONDOR_SCRATCH_DIR", f"/tmp/leyao/x-mc-shape-simultaneous-{os.getpid()}"
    ))
...
def prepare(repo, manifest_path, manifest, output_dir):
    ...
    for category in CATEGORIES:
        ...
        expression = (
            "PrepareXMCShapeFitScanCache.C++(" +
            ",".join(f'"{root_string(value)}"' for value in values) +
            f",{mass_min:.17g},{mass_max:.17g})"
        )
        status = run_root(
            repo / "fitER/cache/PrepareXMCShapeFitScanCache.C", expression,
...
def fit_point(repo, manifest_path, manifest, output_dir, key):
    ...
    mc_shape = contract["signal_mc"]
    sigma_min, sigma_max = mc_shape["sigma1_gev"]["range"]
    if mc_shape["sigma2_gev"]["range"] != [sigma_min, sigma_max]:
        raise RuntimeError("sigma1/sigma2 ranges differ")
    fraction_min, fraction_max = mc_shape["fraction"]["range"]
    a0_min, a0_max = -2.0, 2.0
    a1_min, a1_max = -2.0, 2.0
    mass_bins_float = (mass_max - mass_min) / 0.005
    mass_bins = round(mass_bins_float)
    if abs(mass_bins_float - mass_bins) > 1.e-9:
    ...
    expression = (
        "PbPbXMCShapeSimultaneousYearFit.C++(" +
        ",".join(f'"{root_string(value)}"' for value in values) +
        f",{threshold23:.17g}," +
        ...
        f"{mean_min:.17g},{mean_max:.17g},{sigma_min:.17g},{sigma_max:.17g},"
        f"{fraction_min:.17g},{fraction_max:.17g},{scale_min:.17g},{scale_max:.17g},"
        f"{a0_min:.17g},{a0_max:.17g},{a1_min:.17g},{a1_max:.17g},{mass_bins})"
    )
    status = run_root(
        repo / "fitER/models/PbPbXMCShapeSimultaneousYearFit.C", expression,
        point_dir / "fit.log",
    )
...
def aggregate(repo, manifest_path, manifest, output_dir):
    ...
    fields = [
        ...
        "fit_status", "covQual", "EDM", "shared_mean", "pb23_width_scale",
        "pb23_mc_chi2_ndf_5mev", "pb23_mc_chi2_ndf_1mev",
        "pb23_yield", "pb23_yield_error", "pb23_chi2_ndf", "pb24_width_scale",
        "pb24_mc_chi2_ndf_5mev", "pb24_mc_chi2_ndf_1mev",
        "pb24_yield", "pb24_yield_error", "pb24_chi2_ndf", "q0_joint",
    ...
    result_manifest = {
        ...
        "mc_fit_quality_reporting": {
            "model": "independent-year common-mean double Gaussian",
