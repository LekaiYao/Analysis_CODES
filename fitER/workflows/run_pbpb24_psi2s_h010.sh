...
test -x "${root_bin}"
mkdir -p "${output_dir}"
python3 "${fit_dir}/workflows/prepare_pbpb24_psi2s_h010.py" prepare \
...
while IFS=$'\t' read -r key model_type train_tag target threshold data mc selection; do
  ...
  "${root_bin}" -l -b -q \
    "${fit_dir}/diagnostics/RedrawPsi2SH010Fit.C(\"${model_path}\",\"${model_type}\",${target},${threshold},\"${point_dir}/data_fit_clean.pdf\")" \
    >> "${point_dir}/fit.log" 2>&1
  "${root_bin}" -l -b -q \
    "${fit_dir}/diagnostics/ExportPsi2SH004Summary.C(\"${model_path}\",\"${selection}\",\"${key}\",\"\",\"${point_dir}/fit_summary.csv\")" \
    >> "${point_dir}/fit.log" 2>&1
done < "${output_dir}/working_points.tsv"

python3 "${fit_dir}/workflows/prepare_pbpb24_psi2s_h010.py" aggregate \
  --repo "${repo_dir}" --output-dir "${output_dir}"
