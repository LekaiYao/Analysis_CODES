...
constexpr int kChi2Bins = 12;     // 5 MeV/bin, record only
...
void appendBoundary(std::vector<std::string>& flags, const char* name,
              ...
              int efficiency, const char* year)
{
    ...
    if (!workspace || !baselineFit) {
        throw std::runtime_error("missing baseline peak-fit workspace/result");
    }
    ...
    if (!source || !baselineModel || !mass || !mean || !wideSigma ||
        ...
        throw std::runtime_error("incomplete baseline workspace");
    }
    ...
    // Evaluate the double-Gaussian reference before the shared shape parameters
    // are updated by the Gaussian+CB fit.
    std::unique_ptr<RooPlot> dgChi2Frame(
        mass->frame(Range(kPeakMin, kPeakMax), Bins(kChi2Bins)));
    reduced->plotOn(dgChi2Frame.get(), Name("mc_dg_chi2"),
                    DataError(RooAbsData::SumW2));
    baselineModel->plotOn(dgChi2Frame.get(), Name("model_dg_chi2"),
                          Range("peak_fit"), NormRange("peak_fit"));
    const double dgChi2 = dgChi2Frame->chiSquare(
        "model_dg_chi2", "mc_dg_chi2", baselineFit->floatParsFinal().getSize());
    ...
    if (!fit) throw std::runtime_error("Gaussian+CB peak fit failed");

    std::unique_ptr<RooPlot> cbChi2Frame(
        mass->frame(Range(kPeakMin, kPeakMax), Bins(kChi2Bins)));
    reduced->plotOn(cbChi2Frame.get(), Name("mc_cb_chi2"),
                    DataError(RooAbsData::SumW2));
    model.plotOn(cbChi2Frame.get(), Name("model_cb_chi2"), Range("peak_fit"),
                 NormRange("peak_fit"));
    const double cbChi2 = cbChi2Frame->chiSquare(
        "model_cb_chi2", "mc_cb_chi2", fit->floatParsFinal().getSize());

    std::vector<std::string> boundaries;
    appendBoundary(boundaries, "mean", *mean);
    appendBoundary(boundaries, "cb_sigma", *wideSigma);
    appendBoundary(boundaries, "gaussian_sigma", *narrowSigma);
    appendBoundary(boundaries, "fraction", *fraction);
    appendBoundary(boundaries, "alpha", alpha);
    appendBoundary(boundaries, "n", n);

    gSystem->mkdir(outputDirectory, true);
    TCanvas canvas(Form("c_gauss_cb_%s_%d", year, efficiency), "", 900, 760);
    ...
    TPaveText stats(0.15, 0.57, 0.55, 0.89, "NDC");
    ...
    stats.AddText(Form("#DeltaNLL (CB-G - 2G) = %.3f",
    ...
    RooWorkspace outputWorkspace("ws_x_mc_gauss_cb_diagnostic");
...
void DiagnoseXBaselineMCGaussCB(const char* baselineDirectory,
                                const char* outputDirectory)
{
    ...
    summary << "xeff_percent,year,fit_status,covQual,EDM,nll_gauss_cb,"
