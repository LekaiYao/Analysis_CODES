...
SCRIPT = REPO / "scripts/submit_x_mc_shape_simultaneous_year_fit_manifest.py"
REAL_MANIFEST = Path(
...
def load_module():
    spec = importlib.util.spec_from_file_location("submit_x_mc_shape_simultaneous", SCRIPT)
...
class XMCShapeSubmissionTest(unittest.TestCase):
    def test_official_manifest_validates(self):
        ...
        self.assertTrue(tag.startswith("X_pb23_"))
    ...
    def test_width_scale_contract_is_strict(self):
        ...
        manifest["nominal_fit_contract"]["data_fit"]["category_width_scale"][
            "range"
        ] = [0.9, 1.25]
    ...
    def test_submission_dag_has_prepare_seven_fits_and_final(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch.object(module, "AFS_ROOT", root / "afs"), \
                ...
                task = module.create_submission(REAL_MANIFEST, "test_v2")
